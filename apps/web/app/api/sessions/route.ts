...
export async function GET() {
  ...
  const items = await many(`SELECT s.* FROM sessions s JOIN students st ON st.id=s.student_id WHERE ${scope.canReadAll ? "1=1" : scope.role === "pro" ? "st.coach_user_id=?" : "st.user_id=?"} ORDER BY s.created_at DESC`, scope.canReadAll ? [] : [scope.userId]);
